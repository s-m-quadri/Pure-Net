...
chrome.storage.local.get(
    ...
    function (result) {
        // Remove
        if (result.remove_script) { remove("script"); }
        if (result.remove_style) { remove("style"); }
        if (result.remove_link) { remove("link"); }
        if (result.remove_footer) { remove("footer"); }
        if (result.remove_iframe) { remove("iframe"); }

        // Clean
        if (result.clean_div) { clean_up("div"); }
        if (result.clean_nav) { clean_up("nav"); }
        if (result.clean_span) { clean_up("span"); }
        if (result.clean_cite) { clean_up("cite"); }
        if (result.clean_ul) { clean_up("ul"); }
        if (result.clean_ol) { clean_up("ol"); }
        if (result.clean_li) { clean_up("li"); }

        // Extraction
        if (result.extract_a) { extract_a(); }
        if (result.extract_img) { extract_img(); }
        if (result.extract_button) { extract_button(); }

        // Alert when click on links
        if (result.alert_on_link_click) { alert_on_link_click(); }
...
function remove(tag_name) {
    console.log("Removing '" + tag_name + "' tag with the content inside it.");
    while (document.querySelector(tag_name) != null) {
        selector = document.querySelector(tag_name);
        selector.outerHTML = " ";
    }
}

// Clean specified Tags
function clean_up(tag_name) {
    console.log("Cleaning '" + tag_name + "' tag without affecting content inside it.");
    while (document.querySelector(tag_name) != null) {
        selector = document.querySelector(tag_name);
...
// Extract the links
function extract_a() {
    console.log("Extracting contents inside 'a' tag.");
...
function extract_img() {
    console.log("Extracting contents inside 'img' tag.");
...
// Extract the Buttons 
function extract_button() {
    console.log("Extracting contents inside 'button' tag.");
...
// Alert if clicked on link
function alert_on_link_click() {
    console.log("Adding event listeners on 'a' tags.");
